    ...
    <Card data-testid="queue-item" data-job-id={job.id}>
      <CardContent className="flex flex-col gap-1.5 p-4">
        <div className="flex items-start justify-between gap-3">
          <span className="truncate text-[14px] font-medium text-ink" data-testid="job-title">
            {job.displayTitle}
          </span>
          <div className="flex shrink-0 items-center gap-2">
            <span
              className={cn("rounded-full px-2 py-0.5 text-[11px] font-medium tracking-[0.02em]", STATUS_STYLE[job.status])}
            ...
            {canRetry && (
              <Button variant="secondary" size="sm" onClick={() => onRetry(job.id)} data-testid="retry-button">
                Retry
              </Button>
            )}
          </div>
        </div>
        <span className="truncate font-mono text-[12px] text-ink-subtle" data-testid="job-url">
          {job.url}
        </span>
        {showProgress && job.progress && (
          <div className="mt-1 flex items-center gap-3">
            <Progress value={percent(job.progress.done, job.progress.total)} className="flex-1" />
            <span className="font-mono text-[11px] text-ink-subtle tabular-nums" data-testid="job-progress-text">
