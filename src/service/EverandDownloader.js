...
class EverandDownloader {
  ...
  async listen(url, isEpisode) {
    ...
    const title = await page.evaluate(() => globalThis.Scribd.current_doc.short_title);
  ...
  async series(url) {
    ...
    const totalEpisode = await page.evaluate(() =>
      parseInt(document.querySelector('span[data-e2e="podcast-series-header-total-episodes"]').textContent.replace("episodes", "").trim()),
    );

    // get pages
    const totalPage = await page.evaluate(
      () => [...document.querySelectorAll('div[data-e2e="pagination"] a[aria-label^="Page"]')].at(-1).textContent,
    );
    const bar = new cliProgress.SingleBar({}, cliProgress.Presets.shades_classic);
    ...
    for (let i = 1; i <= totalPage; i++) {
      ...
      let episodes = await page.evaluate(() =>
        [...document.querySelectorAll('div.breakpoint_hide.below a[data-e2e="podcast-episode-player-button"]')].map((x) => x.href),
      );
